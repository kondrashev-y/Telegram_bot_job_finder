...
def chek_number(num: str) -> str:
    number = re.findall(r'\d', num)
    latency_of_number = len(number)

    if 10 > latency_of_number > 12:
        return ''.join(number)
    if number[0] == '9' and latency_of_number == 10:
        number = list('8') + number
    elif number[0] == '7' and latency_of_number == 11:
        number[0] = '8'
    elif number[0] == '8' and latency_of_number == 11:
        pass
    else:
        return ''.join(number)

    normal_number = '8 (9' + ''.join(number[2:4]) + ') ' \
                    + ''.join(number[4:7]) + '-' + ''.join(number[7:9]) + '-' + ''.join(number[9:11])
    normal_number = normal_number.strip()
...
async def root(phone: Phone):
    valid_phone = chek_number(phone.phone)
    print(valid_phone)
    headers = {'Connection': 'keep-alive'}
    return HTMLResponse(content=valid_phone, headers=headers)
